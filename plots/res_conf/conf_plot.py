...
#for i in range(5):
df = pd.read_csv('predict/results-ref/res_0.txt', sep='\t', index_col='gene_id')
...
sns.barplot(x=filtered_df.index, y=filtered_df['classification confidence'], hue=filtered_df['group'], palette='viridis', width=0.6)
...
#plt.show()
plt.savefig(f'predict/results-ref/plot_0.png')
