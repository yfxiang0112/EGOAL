...
for i, name in enumerate(['NADK', 'PAP', 'PPK1', 'PPK2', 'PPX']):
    edges = {}
    ...
    res_df = pd.read_csv(f'predict/results/res_{i}.txt', sep='\t')
    ...
    color_map = {'class1': 'navy', 'class2': 'royalblue', 'class3': 'green'}
    ...
    plt.suptitle(f'Regulation Pathway Graph of Experiment Group {name}', fontsize=20)
    ...
    # 添加图例
    legend_labels = ['Nodes with no loop', 'Nodes with self-loop', 'Overlapping nodes']
    legend_colors = ['royalblue', 'navy', 'green']
    ...
    plt.tight_layout()
    plt.savefig(f'plots/regu_graph/graph_{name}.png')
    plt.show()
